...
const HODLRDenseLU = typeof(lu(Matrix{Float64}(I, 1, 1)))
...
struct hodlr_fact
    ...
    # index information to reconstruct full matrix
    idx_tree::Vector{Any}
    # Reusable numerical factors.  The old implementation stored only the
    # unfactorized matrices, so every solve and log-determinant refactorized
...
function _as_factor_levels(levels, max_level::Integer)
    @assert length(levels) == max_level
...
function _build_hodlr_fact(
    ...
    U_typed = _as_factor_levels(U, max_level_int)
    V_typed = _as_factor_levels(V, max_level_int)
    @assert length(U_typed) == length(V_typed)
    @assert length(leaves_typed) == 2^max_level_int

    cached_leaf_factors = if isnothing(leaf_factors)
        HODLRDenseLU[lu(leaf; check=true) for leaf in leaves_typed]
    else
        @assert length(leaf_factors) == length(leaves_typed)
...
hodlr_fact(max_level, leaves, U, V, idx_tree) =
    _build_hodlr_fact(max_level, leaves, U, V, idx_tree)
...
mutable struct hodlr_nonsymmetric_with_updates
    ...
    Ul::Vector{Vector{Matrix{Float64}}}
    Vl::Vector{Vector{Matrix{Float64}}}
    U_additive::Vector{Vector{Matrix{Float64}}}
    V_additive::Vector{Vector{Matrix{Float64}}}
    idx_tree::Vector{Any}
    function hodlr_nonsymmetric_with_updates(max_level, leaves, U, V, Ul, Vl, U_additive, V_additive, idx_tree)
        new(
            Int(max_level),
            Vector{Matrix{Float64}}(leaves),
            _as_factor_levels(U, max_level),
            _as_factor_levels(V, max_level),
            _as_factor_levels(Ul, max_level),
            _as_factor_levels(Vl, max_level),
            _as_factor_levels(U_additive, max_level),
            _as_factor_levels(V_additive, max_level),
            Vector{Any}(idx_tree),
        )
    end
end
...
function hodlr(mv_query::Function, n::Int64, max_level::Int64, local_rank::Int64, c::Int64)
    ...
    for ii = 1:max_level
        ...
        for kk = 1:Int(num_idx/2)
            tmp_mat = tmp_prod[tmp_idx[2*kk-1], :];
            tmp_matqr = qr(tmp_mat);
            tmp_Qthin = Matrix(tmp_matqr.Q);
            tmp_Qpad = zeros(size(tmp_Qthin, 1), local_rank + c);
            k = min(local_rank + c, size(tmp_Qthin, 2));
            tmp_Qpad[:, 1:k] .= tmp_Qthin[:, 1:k];
            Q_blocks[kk] = tmp_Qpad;
        end
...
function hodlr_grad(param_matvec_grad::Function, n::Int64, max_level::Int64, local_rank::Int64, c::Int64, θ::Vector)
    ...
    for ii = 1:max_level
        ...
        for jj = 1:Int(num_idx/2)
            ...
            tmp_matqr = qr(tmp_mat)
            tmp_Qfull = Matrix(tmp_matqr.Q)
            k = min(local_rank + c, size(tmp_Qfull, 2))
            tmp_Q = tmp_Qfull[:, 1:k]
...
############################################################################################
function _solve_leaf_factor_block(leaf_factors, right_hand_side::AbstractMatrix)
    result = copy(right_hand_side)
    row_start = 1
    for leaf_factor in leaf_factors
        row_end = row_start + size(leaf_factor, 1) - 1
        ldiv!(leaf_factor, @view(result[row_start:row_end, :]))
        row_start = row_end + 1
    end
    @assert row_start == size(result, 1) + 1
    return result
end
...
function hodlr_factorize(A :: hodlr)
    ...
    max_level = A.max_level;
    leaves = A.leaves;
    leaf_factors = HODLRDenseLU[lu(leaf; check=true) for leaf in leaves]
    U = A.U;
    V = A.V;

    # initialize parameters for the new struct
    U_new = Vector{Vector{Matrix{Float64}}}(undef, max_level);
    V_new = Vector{Vector{Matrix{Float64}}}(undef, max_level);

    # preallocate for component matrices

    # upper right block
    U_ur = Vector{Vector{Matrix{Float64}}}(undef, max_level);
    V_ur = Vector{Vector{Matrix{Float64}}}(undef, max_level);
    # lower left block
    U_ll = Vector{Vector{Matrix{Float64}}}(undef, max_level);
    V_ll = Vector{Vector{Matrix{Float64}}}(undef, max_level);
    ...
    for i = max_level:-1:1
        ...
        tmp_U_ur = Vector{Matrix{Float64}}(undef, num_blocks);
        tmp_V_ur = Vector{Matrix{Float64}}(undef, num_blocks);
        tmp_U_ll = Vector{Matrix{Float64}}(undef, num_blocks);
        tmp_V_ll = Vector{Matrix{Float64}}(undef, num_blocks);
        # off-diagonal updates: see (B.3)
        for j = 1:num_blocks
            first_leaf = Int((j - 1) * fact + 1)
            middle_leaf = Int((j - 1) * fact + floor(0.5 * fact))
            last_leaf = Int(j * fact)
            tmp_U_ur[j] = _solve_leaf_factor_block(
                @view(leaf_factors[first_leaf:middle_leaf]),
                tmp_U[j],
            )
            tmp_V_ur[j] = tmp_V[j];
            tmp_U_ll[j] = _solve_leaf_factor_block(
                @view(leaf_factors[(middle_leaf + 1):last_leaf]),
                tmp_V[j],
            )
            tmp_V_ll[j] = tmp_U[j];
        end
    ...
    for i = max_level-1:-1:0
        ...
        for k = i:-1:1
            ...
            tmp_U_ur = Vector{Matrix{Float64}}(undef, length(tp_U_ur));
            tmp_V_ur = Vector{Matrix{Float64}}(undef, length(tp_U_ur));
            tmp_U_ll = Vector{Matrix{Float64}}(undef, length(tp_U_ur));
            tmp_V_ll = Vector{Matrix{Float64}}(undef, length(tp_U_ur));
...
############################################################################################
function _woodbury_solve!(destination, U, V, woodbury_factor)
    correction = transpose(V) * destination
    ldiv!(woodbury_factor, correction)
    mul!(destination, U, correction, -1.0, 1.0)
    return destination
end

function hodlr_solve(A :: hodlr_fact, b :: AbstractVector)
    max_level = A.max_level
    idx_tree = A.idx_tree
    res = copy(b)

    leaf_idx = idx_tree[max_level + 1]
    for block = eachindex(leaf_idx)
        block_result = @view res[leaf_idx[block]]
        ldiv!(A.leaf_factors[block], block_result)
    end

    for level = max_level:-1:1
        level_idx = idx_tree[level]
        for block = eachindex(A.U[level])
            block_result = @view res[level_idx[block]]
            _woodbury_solve!(
                block_result,
                A.U[level][block],
                A.V[level][block],
                A.woodbury_factors[level][block],
            )
        end
    end
    return res
...
function hodlr_solve(A :: hodlr_fact, b :: AbstractMatrix)
    """ 
        Solves linear system where A is in HODLR one-way factorized format. 
        Returns A^-1*b while reusing cached leaf and Woodbury factorizations.
...
@inline function _dense_diagonal_sum(A::AbstractMatrix)
    result = zero(eltype(A))
    @inbounds @simd for i = 1:min(size(A, 1), size(A, 2))
        result += A[i, i]
    end
    return result
end

@inline function _frobenius_inner(A::AbstractMatrix, B::AbstractMatrix)
...
function hodlr_tr(A :: hodlr)
    ...
    """
    # HODLR off-diagonal blocks never contribute to the trace.
    res = 0.0
    @inbounds for leaf in A.leaves
        res += _dense_diagonal_sum(leaf)
    end
    return res
end
...
function hodlr_tr(A :: hodlr_nonsymmetric_with_updates)
    ...
    # tr(U*V') = sum(U .* V).  The Frobenius inner product avoids
    # allocating V' * U and reduces O(n*r^2) work to O(n*r).
    @assert length(A.V_additive) == length(A.U_additive)
    @assert length(A.U_additive) == A.max_level
    @inbounds for k = 1:A.max_level
        Uk_additive = A.U_additive[k]
        Vk_additive = A.V_additive[k]
        @assert length(Uk_additive) == length(Vk_additive)
        for i = eachindex(Uk_additive)
            res += _frobenius_inner(Vk_additive[i], Uk_additive[i])
        end
    end
    return res
end
...
function hodlr_logdet(A :: hodlr_fact)
    """
        Return the log absolute determinant cached by `hodlr_factorize`.
    """
    return A.logabsdet_value
end
...
############################################################################################
function hodlr_prod(A, x::AbstractVector)
    return vec(hodlr_prod(A, reshape(x, :, 1)))
end

function hodlr_prod(A, x::AbstractMatrix)
    """
    ...
    sol = zeros(promote_type(eltype(A.leaves[1]), eltype(x)), size(x))
    # diagonal
    leave_idx = A.idx_tree[end]
    for i = eachindex(leave_idx)
        idx = leave_idx[i]
        mul!(@view(sol[idx, :]), A.leaves[i], @view(x[idx, :]))
    end
    # off-diagonal
    for level = max_level:-1:1
        Ul = A.U[level]
        ...
        for ii = eachindex(Ul)
            Uli = Ul[ii]
            Vli = Vl[ii]
            idx1 = A.idx_tree[level][2 * (ii - 1) + 1]
            idx2 = A.idx_tree[level][2 * ii]

            upper_workspace = transpose(Vli) * @view(x[idx2, :])
            mul!(@view(sol[idx1, :]), Uli, upper_workspace, 1.0, 1.0)

            lower_workspace = transpose(Uli) * @view(x[idx1, :])
            mul!(@view(sol[idx2, :]), Vli, lower_workspace, 1.0, 1.0)
        end
    end
    return sol
end
...
function hodlr_invmult(A :: hodlr_fact, B :: hodlr)
    ...
    return hodlr_invmult_transposed(hodlr_fact_transpose(A), B)
end

function hodlr_invmult_transposed(A :: hodlr_fact, B :: hodlr)
    """
        Compute A_original^-1 * B when `A` is the cached transposed one-way
        factorization returned by `hodlr_fact_transpose`.  This entry point lets
        callers reuse that transpose across all covariance derivatives.
    """
    ...
    U_additive = Vector{Vector{Matrix{Float64}}}(undef, max_level)
    V_additive = Vector{Vector{Matrix{Float64}}}(undef, max_level)
    ...
    level_one_product = hodlr_prod(B, A.V[1][1])
    level_one_rhs = Matrix(transpose(level_one_product))
    ldiv!(A.woodbury_factors[1][1], level_one_rhs)
    V_additive[1] = [Matrix(transpose(level_one_rhs))]
    # level 2 onwards
    for l = 2:max_level
        ...
        for k = 1:l-1
            ...
            U_additive[k] = block_inv_update_cached(
                U_additive[k],
                A.U[l],
                A.V[l],
                A.woodbury_factors[l],
            )
            #@show l, k 
        ...
        tmp_U = Vector{Matrix{Float64}}(undef, num_factors)
        tmp_V = Vector{Matrix{Float64}}(undef, num_factors)
        for j = 1:num_factors
            tmp_U[j] = -A.U[l][j];
            #@show j, norm(tmp_U[j])
            ...
            #@show j, norm(tp)
            tp_rhs = Matrix(transpose(tp))
            ldiv!(A.woodbury_factors[l][j], tp_rhs)
            tmp_V[j] = Matrix(transpose(tp_rhs))
        end
        # update
        U_additive[l] = tmp_U;
        V_additive[l] = tmp_V;
    ...
    for j = eachindex(B.leaves)
        output_B.leaves[j] = A.leaf_factors[j] \ output_B.leaves[j]
    ...
    for k = max_level:-1:1
        ...
        tmp = block_inv_update2_cached(tmp, A.leaf_factors)
        #@show k, [norm(tmp[i]) for i = 1:length(tmp)]
        # unpack 
        output_B.U[k] = tmp[1:2:end];
        output_B.Ul[k] = tmp[2:2:end];
    ...
    for k = max_level:-1:1
        ...
        tp = block_inv_update2_cached(tp, A.leaf_factors)
        #@show k, [norm(tp[i]) for i = 1:length(tp)]
        U_additive[k] = tp; 
    end
    # output struct
...
function Base.:\(A_fact::hodlr_fact, B::Union{hodlr, hodlr_nonsymmetric})#::Union{hodlr, hodlr_nonsymmetric}
    ...
    for lvl = 1:max_level
        if lvl == 1
            ...
            new_V_rhs = Matrix{Float64}(tmp_V' * ret)
            ldiv!(A_fact.woodbury_factors[lvl][1], new_V_rhs)
            new_V = Matrix{Float64}(transpose(new_V_rhs))
        ...
        for i = 1:2^(lvl - 1)
            ...
            new_V_rhs = Matrix{Float64}(tmp_V' * diag_block)
            ldiv!(A_fact.woodbury_factors[lvl][i], new_V_rhs)
            new_V = Matrix{Float64}(transpose(new_V_rhs))
        ...
        for i = lvl - 1:-1:1
            ...
            tmp_off_diag_l = block_inv_update_cached(
                tmp_off_diag_l,
                A_fact.U[lvl],
                A_fact.V[lvl],
                A_fact.woodbury_factors[lvl],
            )
    ...
    for i = 1:length(ret.leaves)
        ret.leaves[i] = A_fact.leaf_factors[i] \ ret.leaves[i]
    end
    for i = 1:max_level
        tmp_off_diag_l = [];
        tmp_off_diag_r = [];
        ...
        # Update the off-diagonal low-rank factors 
        tmp_off_diag_l = block_inv_update2_cached(
            tmp_off_diag_l,
            A_fact.leaf_factors,
        )
        tmp_off_diag_r = tmp_off_diag_r;
...
function block_inv_update_cached(
    Uk_additive::Array,
    Ul::Array,
    Vl::Array,
    woodbury_factors::Array,
)
    """Apply block Woodbury inverses using precomputed small LU factors."""
    num_fine_blocks = length(Ul)
    @assert length(Vl) == num_fine_blocks
    @assert length(woodbury_factors) == num_fine_blocks
    num_coarse_blocks = length(Uk_additive)
    factor = Int(num_fine_blocks / num_coarse_blocks)
    n_fine = size(Ul[1], 1)
    ...
    for i = eachindex(Uk_additive)
        tmp_U = Uk_additive[i]
        for j = 1:factor
            fine_block = Int((i - 1) * factor + j)
            idx = Int((j - 1) * n_fine + 1):Int(j * n_fine)
            _woodbury_solve!(
                @view(tmp_U[idx, :]),
                Ul[fine_block],
                Vl[fine_block],
                woodbury_factors[fine_block],
            )
        end
        Uk_additive[i] = tmp_U
    end
...
function block_inv_update2_cached(Uk_additive::Array, leaf_factors::Array)
    """Apply cached leaf factorizations to blockwise low-rank factors."""
    num_fine_blocks = length(leaf_factors)
    num_coarse_blocks = length(Uk_additive)
    factor = Int(num_fine_blocks / num_coarse_blocks)

    for i = eachindex(Uk_additive)
        tmp_U = Uk_additive[i]
        n_fine = div(size(tmp_U, 1), factor)
        for j = 1:factor
            fine_block = Int((i - 1) * factor + j)
            idx = Int((j - 1) * n_fine + 1):Int(j * n_fine)
            ldiv!(leaf_factors[fine_block], @view(tmp_U[idx, :]))
        end
        Uk_additive[i] = tmp_U
    end
    return Uk_additive
