...
all_nmc_exponents = [8, 9, 10, 11, 12, 13]; # can also be a list

for ii = eachindex(all_nmc_exponents)
    nmc_exponent = all_nmc_exponents[ii];
    println("Running NMC exponent $nmc_exponent ...")
    # generate Matern source
    nmc = Int(2^nmc_exponent);
    ...
    all_degs = [8, 9, 10, 11, 12];  # can also be a list
    for kk = eachindex(all_degs)
        ...
        for ll = eachindex(all_basis_names)
            ...
            filename = "./data/" * 
                "adv1d_basiscoefs_Nmc$(nmc_exponent)_Deg$(p)_Basis$(basis_name)_mlecoefs.txt";
            # save
            writedlm(filename, θ_mle);
        end # loop over basis
